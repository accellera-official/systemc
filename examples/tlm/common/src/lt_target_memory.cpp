  ...
  , sc_core::sc_time         &delay_time)       ///< time it should take for transport
{
  ...
  if (phase != tlm::BEGIN_REQ) {
    REPORT_FATAL(report_source, "nb_transport", "phase not BEGIN_REQ");
  }
  ...
  switch (command)
  {
    default:
    {
      REPORT_FATAL(report_source, "nb_transport", "invalid command");
      break;
    }
    ...
    case tlm::TLM_WRITE_COMMAND:
    {
      ...
      REPORT_INFO(report_source, "nb_transport", msg.str());
...
)
{
    ...
    //REPORT_FATAL(report_source, "transport_dbg", "routine not implemented");
  return 0;
}
...
)
{
    ...
    // throw an error:
    // REPORT_FATAL(report_source, "get_direct_mem_ptr", "routine not implemented");
    return false;
}
