...
public:	 
	...
	tlm_initiator(sc_core::sc_module_name name, tlm::tlm_endianness endianness, bool first)
	...
	void main()
	{
		...
		std::cout << std::endl;
		std::cout << name() << " : TEST 1 : WRITE and READ to SLAVE LE " << std::endl;

		std::cout << name() << " : Send write request: A = 0x" 
			<< std::hex << ADDRESS_SLAVE_LE << ", D = 0x" << wr_data << std::dec
            << " @ " << sc_core::sc_time_stamp() << std::endl;
		writeBurst(ADDRESS_SLAVE_LE, reinterpret_cast<unsigned char*>(&wr_data), 4);
		std::cout << std::endl;

		std::cout << name() << " : Send read request: A = 0x" << std::hex << ADDRESS_SLAVE_LE << std::dec
               << " @ " << sc_core::sc_time_stamp() << std::endl;
		readBurst(ADDRESS_SLAVE_LE, reinterpret_cast<unsigned char*>(&rd_data), 4);
		std::cout << name() << " : Receive read request: A = 0x"
               << std::hex << ADDRESS_SLAVE_LE << ", D = 0x" << std::hex << rd_data << std::dec
               << " @ " << sc_core::sc_time_stamp() << std::endl;
		std::cout << std::endl;
		...
		std::cout << std::endl;
		std::cout << name() << " : TEST 2 : WRITE and READ to SLAVE BE " << std::endl;

		std::cout << name() << " : Send write request: A = 0x" 
			<< std::hex << ADDRESS_SLAVE_BE << ", D = 0x" << wr_data << std::dec
            << " @ " << sc_core::sc_time_stamp() << std::endl;
		writeBurst(ADDRESS_SLAVE_BE, reinterpret_cast<unsigned char*>(&wr_data), 4);
		std::cout << std::endl;

		std::cout << name() << " : Send read request: A = 0x" << std::hex << ADDRESS_SLAVE_BE << std::dec
               << " @ " << sc_core::sc_time_stamp() << std::endl;
		readBurst(ADDRESS_SLAVE_BE, reinterpret_cast<unsigned char*>(&rd_data), 4);
		std::cout << name() << " : Receive read request: A = 0x"
               << std::hex << ADDRESS_SLAVE_BE << ", D = 0x" << std::hex << rd_data << std::dec
               << " @ " << sc_core::sc_time_stamp() << std::endl;
		std::cout << std::endl;
		...
		if(m_first)
		{
			...
			std::cout << std::endl;
			std::cout << name() << " : TEST 3 : MIX WRITE and READ " << std::endl;

			std::cout << name() << " : Send write request: A = 0x" 
				<< std::hex << ADDRESS_COMMON_SLAVE_LE << ", D = 0x" << wr_data << std::dec
                << " @ " << sc_core::sc_time_stamp() << std::endl;
			writeBurst(ADDRESS_COMMON_SLAVE_LE , reinterpret_cast<unsigned char*>(&wr_data), 4);
			std::cout << std::endl;

			std::cout << name() << " : Send write request: A = 0x" 
				<< std::hex << ADDRESS_COMMON_SLAVE_BE << ", D = 0x" << wr_data << std::dec
                << " @ " << sc_core::sc_time_stamp() << std::endl;
			writeBurst(ADDRESS_COMMON_SLAVE_BE , reinterpret_cast<unsigned char*>(&wr_data), 4);
			std::cout << std::endl;
		}
		else
		{
			...
			std::cout << name() << " : Send read request: A = 0x" << std::hex << ADDRESS_COMMON_SLAVE_LE  << std::dec
                << " @ " << sc_core::sc_time_stamp() << std::endl;
			readBurst(ADDRESS_COMMON_SLAVE_LE , reinterpret_cast<unsigned char*>(&rd_data), 4);
			std::cout << name() << " : Receive read request: A = 0x"
                << std::hex << ADDRESS_COMMON_SLAVE_LE  << ", D = 0x" << std::hex << rd_data << std::dec
                << " @ " << sc_core::sc_time_stamp() << std::endl;
			std::cout << std::endl;
			...
			std::cout << name() << " : Send read request: A = 0x" << std::hex << ADDRESS_COMMON_SLAVE_BE  << std::dec
                << " @ " << sc_core::sc_time_stamp() << std::endl;
			readBurst(ADDRESS_COMMON_SLAVE_BE , reinterpret_cast<unsigned char*>(&rd2_data), 4);
			std::cout << name() << " : Receive read request: A = 0x"
                << std::hex << ADDRESS_COMMON_SLAVE_BE  << ", D = 0x" << std::hex << rd2_data << std::dec
                << " @ " << sc_core::sc_time_stamp() << std::endl;
			std::cout << std::endl;
			
			std::cout << name() << " : SLAVE LE : WR_DATA equal to RD_DATA : ";
			if(wr_data == rd2_data) std::cout << "OK"; else std::cout << "ERROR";
			std::cout << std::endl << std::endl;
		}
