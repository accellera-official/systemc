...
//
// Notes:
//   (1) The m_explicit_reset field is used to handle synchronous resets that
...
inline void sc_thread_process::suspend_me()
{
    ...
    // IF THE MODAL SYNCHRONOUS RESET IS ACTIVE THROW AN EXCEPTION:

    if ( m_explicit_reset || m_active_reset_n || m_active_areset_n )
    {
        throw sc_user();
    }
