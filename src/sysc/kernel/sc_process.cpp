...
// These two are immutable empty fallbacks returned by sc_process_handle::
// get_child_events() / get_child_objects() when the handle is empty.
// Read-only after default construction, so no thread_local is needed.
