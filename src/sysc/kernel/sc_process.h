...
//   (3) The m_explicit_reset field is used to handle synchronous resets that
...
//          * ready - process executing or on the execution queue
//          * suspended - process was suspended while in waiting
//          * suspended_pending - process was suspended while ready 
...
//          * ready + DISABLE -> disabled_pending
//          * ready + ENABLE -> ready
//          * ready + EXECUTE -> ready
//          * ready + EVENT -> ready
//          * ready + RESUME -> ready
//          * ready + SUSPEND -> suspended_pending
//          * ready + WAIT -> waiting
//       
...
//          * suspended + EVENT -> suspended_pending
//          * suspended + RESUME -> waiting
//          * suspended + SUSPEND -> suspended
//          * suspended + WAIT -> illegal
//       
//        suspended_pending State Transitions:
//          * suspended_pending + DISABLE -> disabled_suspended
//          * suspended_pending + ENABLE -> suspended_pending
//          * suspended_pending + EXECUTE -> illegal
//          * suspended_pending + EVENT -> suspended_pending
//          * suspended_pending + RESUME -> ready
//          * suspended_pending + SUSPEND -> suspended_pending
//          * suspended_pending + WAIT -> illegal
//       
...
class sc_process_b : public sc_object { 
  ...
  public:
    ...
    enum process_state {
        ps_disabled = 0,
        ps_disabled_pending,
        ps_disabled_suspended,
        ps_normal,
        ps_suspended,
        ps_suspended_pending,
        ps_zombie
    };
    enum reset_type {
...
// called this method won't be either. We get called any time the value 
// changes since we either need to throw an exception or clear one.
...
{       
    ...
    // Clear the throw type if there are no active resets, from signals or
    // modally.

    if ( m_throw_type == THROW_RESET && m_explicit_reset == false &&
         m_active_areset_n == 0 && m_active_reset_n == 0 )
    {
