...
struct Top: sc_module
{
  ...
  {
    cout << "target5 called at " << sc_time_stamp() << " with count = " << count << endl;
    ...
    switch (count)
    {
      ...
      case 23: sc_assert( sc_time_stamp() == sc_time(650, SC_NS) ); f25=1; break;
      default: sc_assert( false ); break;
    }
    ...
    for (;;)
    {
      try {
        ...
        cout << "target5 awoke at " << sc_time_stamp() << " with count = " << count << endl;

        switch (count)
        {
          case 20: sc_assert( sc_time_stamp() == sc_time(620, SC_NS) ); f21=1; break;
          default: sc_assert( false ); break; // FAILS count=23 time=650
        }
      }
      catch (const std::exception& ex) {

        cout << "target5 catch at " << sc_time_stamp() << " with count = " << count
             << ", sc_is_unwinding() = " << sc_is_unwinding() << endl;
