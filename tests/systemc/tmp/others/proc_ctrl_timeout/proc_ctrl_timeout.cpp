...
struct Top: sc_module
{
  ...
  void calling()
  {
    ...
    count = 11;
    t5.reset(); // Following reset, target5 should not be sensitive to event_and_list
    wait(SC_ZERO_TIME);
    
    count = 12;
    ev3.notify(3, SC_NS);  // FAILS - target5 still sensitive to event list !!!!!!
    ev2.notify(2, SC_NS);
    ev1.notify(1, SC_NS);
    wait(10, SC_NS);
  ...
  void target5()
  {
    ...
    switch (count)
    {
        ...
        case 13: sc_assert( sc_time_stamp() == sc_time(420, SC_NS) ); f20=1; break;
        default: sc_assert( false ); break;
    }
    if (count < 13)
    {
      ...
      next_trigger(10, SC_NS, ev1 & ev2 & ev3);
  }
