...
void test_fx(U a_mul, U b_init, U b_mul, char const *t_name, char const *u_name)
{
  ...
  for (U i = 1; i < 4; ++i) {
    ...
    // to a type large enough to perform arithmetic means we can be subject to
    // implicit upcasts. That's minimized by having all of the values of type U,
    // but if U is smaller than int it can still be implicitly upcast, ergo the
    // outer cast.
